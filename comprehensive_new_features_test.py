...
Testing the specific features mentioned in the Arabic review request with proper inventory setup:
...
class ComprehensiveNewFeaturesAPITester:
    ...
    def setup_inventory_for_testing(self):
        """Setup inventory items needed for raw material creation"""
        print("\n=== Setting Up Inventory for Testing ===")
        ...
        for item_data in inventory_items:
            try:
                ...
                if response.status_code == 200:
                    item = response.json()
                    self.created_data['inventory_items'].append(item['id'])
                    created_count += 1
                else:
                    print(f"   Failed to create inventory item: {response.status_code} - {response.text}")
            except Exception as e:
                print(f"   Exception creating inventory item: {str(e)}")
        
        self.log_test("Setup inventory items", created_count > 0, 
                     f"Created {created_count} inventory items for testing")
    ...
    def test_w_type_wall_height(self):
        ...
        try:
            ...
            w_product_data = {
                ...
                "inner_diameter": 25.0,
                "outer_diameter": 45.0,
                "height": 8.0,  # This represents wall height for W types
                "quantity": 10,
                "unit_price": 15.0
            }
            ...
            if response.status_code == 200:
                ...
                # Check if wall height is properly saved
            ...
            if customer_response.status_code == 200:
                ...
                if invoice_response.status_code == 200:
                    ...
                    if item.get('height') == 8.0 and item.get('seal_type') == 'W1':
                        ...
                        # Check if it appears in work order
                        ...
                        if work_order_response.status_code == 200:
                            ...
                            # Find our invoice in the work order
                            our_invoice = next((inv for inv in work_order_invoices if inv.get('id') == invoice['id']), None)
                            if our_invoice and our_invoice['items'][0].get('height') == 8.0:
                                self.log_test("W Type wall height in work order", True, 
                                            f"Wall height preserved in work order: {our_invoice['items'][0].get('height')}mm")
                            else:
                                self.log_test("W Type wall height in work order", False, 
                                            f"Wall height not preserved in work order")
                        else:
                            self.log_test("W Type wall height in work order", False, 
                                        f"Work order not found: HTTP {work_order_response.status_code}")
                    else:
    ...
    def test_inventory_editing(self):
        ...
        # Test editing raw materials (with proper inventory setup)
        try:
            # First create a raw material (this should work now with inventory setup)
            raw_material_data = {
                "material_type": "NBR",
                "inner_diameter": 20.0,
                "outer_diameter": 40.0,
            ...
            if create_response.status_code == 200:
                ...
                self.log_test("Edit raw material", False, 
                            f"Raw material creation failed: HTTP {create_response.status_code} - {create_response.text}")
            ...
            if len(created_materials) >= 1:
                ...
                if compat_response.status_code == 200:
                    ...
                    suitable_materials = [m for m in compatible_materials 
                                        if m['inner_diameter'] <= 18.0 and m['outer_diameter'] >= 38.0]
                ...
                self.log_test("Create multiple NBR materials", False, 
                            f"Only created {len(created_materials)} out of 2 materials")
            ...
            if supplier_response.status_code == 200:
                ...
                if product_response.status_code == 200:
                    ...
                    if customer_response.status_code == 200:
                        ...
                        # Create invoice with local product using new format (product_size - product_type)
                        invoice_data = {
                            ...
                            "items": [{
                                ...
                                "local_product_details": {
                                    "name": "خاتم زيت محلي اختبار",
                                    "supplier": "مورد المنتجات المحلية",
                                    "product_type": "خاتم زيت",  # This should be the product type
                                    "product_size": "50 مم"  # This should be the product size
                                }
                            }],
                            "payment_method": "نقدي"
                        }
                        ...
                        if invoice_response.status_code == 200:
                            ...
                            if (local_details.get('product_type') == 'خاتم زيت' and 
                                local_details.get('product_size') == '50 مم'):
                                self.log_test("Local product display format", True, 
                                            f"Local product saved with correct format: {local_details.get('product_size')} - {local_details.get('product_type')}")
                            else:
                            ...
                            if get_invoice_response.status_code == 200:
                                ...
                                if (retrieved_details.get('product_type') == 'خاتم زيت' and 
                                    retrieved_details.get('product_size') == '50 مم'):
    ...
    def test_complete_workflow(self):
        ...
        try:
            ...
            if customer_response.status_code == 200:
                ...
                # 2. Create raw material (should work now with inventory setup)
                raw_material_data = {
                    "material_type": "BUR",
                    "inner_diameter": 18.0,
                    "outer_diameter": 38.0,
                ...
                if material_response.status_code == 200:
                    ...
                    self.log_test("Complete workflow integration", False, 
                                f"Raw material creation failed: HTTP {material_response.status_code} - {material_response.text}")
            else:
                self.log_test("Complete workflow integration", False, 
                            f"Customer creation failed: HTTP {customer_response.status_code}")
    ...
    def cleanup_test_data(self):
        ...
        for item_id in self.created_data['inventory_items']:
            try:
                response = self.session.delete(f"{BACKEND_URL}/inventory/{item_id}")
                if response.status_code == 200:
                    cleanup_count += 1
            except:
                pass
    ...
    def run_all_tests(self):
        ...
        if not self.setup_inventory_for_testing():
            print("Failed to setup inventory. Some tests may fail.")
