...
class LocalProductFixesTest:
    ...
    async def create_test_customer(self):
        ...
        try:
            ...
            async with self.session.post(f"{BACKEND_URL}/customers", json=customer_data) as response:
                if response.status == 200:
                    ...
                    self.log_result("إنشاء عميل اختبار", False, f"فشل إنشاء العميل: {response.status}", error_text)
                    return False
    ...
    async def test_1_local_product_invoice_creation(self):
        """
        ...
        try:
            ...
            async with self.session.post(f"{BACKEND_URL}/invoices", json=invoice_data) as response:
                response_text = await response.text()
                
                if response.status == 200:
                    invoice = await response.json() if response.content_type == 'application/json' else None
                    if invoice:
                        ...
                        self.log_result("إنشاء فاتورة مع منتج محلي", False, "استجابة فارغة من الخادم", response_text)
                        return False
                else:
                    self.log_result(
                        "إنشاء فاتورة مع منتج محلي", 
                        False, 
                        f"فشل إنشاء الفاتورة - HTTP {response.status}",
                        response_text
                    )
                    return False
    ...
    async def test_2_verify_invoice_data_integrity(self):
        """
        ...
        try:
            async with self.session.get(f"{BACKEND_URL}/invoices/{self.created_invoice_id}") as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    self.log_result("التحقق من سلامة البيانات", False, f"فشل استرجاع الفاتورة: {response.status}", error_text)
                    return False
    ...
    async def test_3_invoice_edit_functionality(self):
        """
        ...
        try:
            ...
            async with self.session.get(f"{BACKEND_URL}/invoices/{self.created_invoice_id}") as response:
                ...
                current_invoice = await response.json()
            ...
            # إرسال التحديث
            async with self.session.put(f"{BACKEND_URL}/invoices/{self.created_invoice_id}", json=update_data) as response:
                if response.status == 200:
                    self.log_result("تعديل الفاتورة", True, "تم تعديل الفاتورة بنجاح")
                    
                    # التحقق من حفظ التعديل
                    async with self.session.get(f"{BACKEND_URL}/invoices/{self.created_invoice_id}") as verify_response:
                        if verify_response.status == 200:
                            updated_invoice = await verify_response.json()
                    ...
                    error_text = await response.text()
                    self.log_result("تعديل الفاتورة", False, f"فشل في تعديل الفاتورة: {response.status}", error_text)
                    return False
    ...
    async def cleanup_test_data(self):
        ...
        try:
            ...
            if self.created_invoice_id:
                async with self.session.delete(f"{BACKEND_URL}/invoices/{self.created_invoice_id}") as response:
                    if response.status == 200:
                        self.log_result("تنظيف البيانات", True, "تم حذف الفاتورة التجريبية")
                    
            # Delete test customer
            if self.created_customer_id:
                async with self.session.delete(f"{BACKEND_URL}/customers/{self.created_customer_id}") as response:
                    if response.status == 200:
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            test1_success = await self.test_1_local_product_invoice_creation()
            
            # Test 2: Verify data integrity (no size duplication)
            test2_success = await self.test_2_verify_invoice_data_integrity()
            
            # Test 3: Invoice edit functionality
            test3_success = await self.test_3_invoice_edit_functionality()
